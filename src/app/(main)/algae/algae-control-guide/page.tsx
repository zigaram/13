...
export async function generateMetadata(): Promise<Metadata> {
  const article = await getArticleBySlug('algae-control-guide');
  if (!article) return {};
  return {
...
export default async function AlgaeControlPage() {
  const article = await getArticleBySlug('algae-control-guide');
  if (!article) notFound();

  const breadcrumbs = buildBreadcrumbs([
    { label: 'Algae Control', href: '/algae' },
    { label: 'Algae Control Guide', href: `/algae/${article.slug}` },
  ]);
